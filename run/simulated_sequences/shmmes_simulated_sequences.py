...
if __name__ == "__main__":
    ...
    external_signal = pd.read_csv(
        "/hmm_with_external_signals/data/f1_main_10_sequences.csv", index_col=0
    )["eu_female_top_325"]
    ...
    w_train = external_signal[:train_length]
    w_test = external_signal[train_length : train_length + test_length]

    original_model = SHMMES(nb_hidden_states=2, season=52)
    pi = np.array([0.0, 1.0])
    sigma = np.array([0.25, 0.5])
    delta = np.array([[3.0, 0.8, 2.5, 4.0], [-1.1, -0.1, -1.5, 3.5]])
    omega = np.array([[[0.5, 0.9, 0.7, 0.5]], [[-2, -0.2, -0.6, 0.7]]])
    original_model.assign_param(pi, delta, sigma, omega)
    result = original_model.simulate_xy(
        horizon=train_length + test_length,
        w=external_signal[: train_length + test_length],
    )
