...
class SHMM(Hidden_Markov_Model):
    """
    Class defining a seasonal HMM.
    """
    ...
    def define_param(self) -> None:
        ...
        self.delta_var = tf.Variable(
            [[0.5 for i in range(3)] for j in range(self.K)], dtype=tf.float64
        )
            ...
            tf.math.reduce_sum(
                ...
                * tf.expand_dims(
                    tf.concat(
                        [
                            tf.expand_dims(
                                tf.ones(t.shape[0], dtype=tf.float64), axis=1
                            ),
