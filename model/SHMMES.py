...
class SHMMES(Hidden_Markov_Model):
    """
    Class defining a seasonal HMM with external signals.
    """
...
class SHMMES_window(Hidden_Markov_Model):
    """
    ...
    def __init__(
        ...
        """
        Instantiate a SHMMES model with gaussian emission laws and discrete hidden states.
...
class S2HMMES_window(Hidden_Markov_Model):
    """
    Class defining a autoregressive seasonal HMM with external signals.
    """
    ...
    def define_param(self) -> None:
        ...
        self.pi_var = tf.Variable([0.5 for i in range(self.K - 1)], dtype=tf.float64)
        self.delta_var = tf.Variable(
            [[0.5 for i in range(5 + self.past_dependency0)] for j in range(self.K)],
            dtype=tf.float64,
        )
        self.sigma_var = tf.Variable([0.5 for i in range(self.K)], dtype=tf.float64)
        self.omega_var = tf.Variable(
                ...
                * tf.expand_dims(
                    tf.concat(
                        [
                            ...
                            tf.math.cos((2 * np.pi * t) / self.season),
                            tf.math.sin((2 * np.pi * t) / self.season),
                            tf.math.cos((4 * np.pi * t) / self.season),
                            tf.math.sin((4 * np.pi * t) / self.season),
                        ],
                        axis=1,
                    ),
                    axis=1,
...
class S3HMMES_window(Hidden_Markov_Model):
    """
    Class defining a autoregressive seasonal HMM with external signals.
    """
    ...
    def define_param(self) -> None:
        ...
        self.pi_var = tf.Variable([0.5 for i in range(self.K - 1)], dtype=tf.float64)
        self.delta_var = tf.Variable(
            [[0.5 for i in range(7 + self.past_dependency0)] for j in range(self.K)],
            dtype=tf.float64,
        )
        self.sigma_var = tf.Variable([0.5 for i in range(self.K)], dtype=tf.float64)
        self.omega_var = tf.Variable(
                ...
                * tf.expand_dims(
                    tf.concat(
                        [
                            ...
                            tf.math.cos((4 * np.pi * t) / self.season),
                            tf.math.sin((4 * np.pi * t) / self.season),
                            tf.math.cos((8 * np.pi * t) / self.season),
                            tf.math.sin((8 * np.pi * t) / self.season),
                        ],
                        axis=1,
                    ),
                    axis=1,
