...
class HMM(Hidden_Markov_Model):
    """
    Class defining a simple HMM model.
    """
    ...
    def define_param(self) -> None:
        ...
        self.delta_var = tf.Variable([[0.5] for i in range(self.K)], dtype=tf.float64)
    ...
    def tp(self, t: tf.Tensor, w: tf.Tensor, y_past: tf.Tensor) -> tf.Tensor:
        ...
        y = tf.expand_dims(tf.ones_like(t, dtype=tf.float64), axis=1)
